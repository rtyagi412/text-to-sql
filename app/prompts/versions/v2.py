...
- ambiguities: empty list when nothing is missing or unclear. Otherwise one entry per issue, each
  ...
  not just the first one you notice:
    ...
    - AMBIGUOUS_FIELD: output_fields is non-empty, but at least one requested field is too vague to
      map to a concrete attribute (e.g. "Card Details" — which fields, specifically?).
                ...
                "ambiguities": [
                    {
                        ...
                        "description": "The requested field \"Card Details\" is too vague to map to a concrete attribute — it's unclear which specific card details should be included.",
                        "clarification_question": "Which specific card detail fields should be included in place of the general \"Card Details\" field (e.g. expiry date, masked PAN, issuing bank)?",
                    },
                ],
                "status": "NEEDS_CLARIFICATION",
            },
...
PROMPT_VERSION = PromptVersion(
    id="v2",
    description=(
        "RITM extraction prompt, revised after auditing v1 against the expanded 15-case "
        "golden set (tiers 1-5 + 5 clarify cases). Adds three rules v1 lacked: (1) the "
        "monetary '_units' suffix convention for amount/price filter fields, since it's a "
        "storage convention not inferable from ticket wording; (2) the entity-name -> "
        "PascalCase field-prefix normalization rule, stated explicitly with examples; "
        "(3) ambiguities are additive - a ticket can carry multiple independent categories "
        "at once. Scoped to query-formation fields only: no intent classification, report "
        "delivery/frequency, existing-report reference, or standalone time_range concept — "
        "every reporting window (absolute or relative) is just a filter on a date/timestamp "
        "attribute, with relative windows ('last N days') becoming a GREATER_THAN_OR_EQUAL "
        "filter with a '-N UNIT' value instead of an invented absolute date. The ambiguity "
        "category list is trimmed to the ones that actually block forming a query "
        "(MISSING_ENTITY, MISSING_OUTPUT_FIELDS, MISSING_REPORT_CRITERIA, AMBIGUOUS_FIELD, "
        "AMBIGUOUS_FILTER, OTHER) — AMBIGUOUS_TIME_RANGE was folded into AMBIGUOUS_FILTER "
        "since an unquantifiable reporting window is just an unquantifiable filter condition "
        "now. 8 few-shot examples, all grounded in real tickets: 3 kept from v1 (RITM0030001 "
        "baseline EQUALS, RITM0030004 IN + literal-date BETWEEN filter, RITM0030008 IS_NULL + "
        "relative-date filter + PascalCase two-entity case), RITM0030012 (MISSING_OUTPUT_FIELDS, "
        "filters still extracted from criteria alone) plus 4 more: RITM0030006 "
        "(GREATER_THAN+_units), RITM0030007 (BETWEEN+IN+_units, entity included via filter "
        "alone with zero of its own fields displayed), RITM0030015 (AMBIGUOUS_FIELD), "
        "RITM0030016 (AMBIGUOUS_FILTER over an unquantifiable time window). RITM0030009/0030010 "
        "(the 3-8 entity join chains) are deliberately left out of the few-shots so the eval "
        "still tests generalization to longer entity chains rather than memorized examples. "
        "System prompt separately instructs scanning output_fields/report_criteria item by "
        "item rather than reasoning about the ticket holistically, to improve entity recall "
        "on long chains without spending a few-shot slot on either holdout ticket."
    ),
